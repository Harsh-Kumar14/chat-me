...
import {PanelLeftIcon, PenSquare,Plus,MessageSquare,Menu,X, User,LogOut,PanelRight} from "lucide-react"
    ...
    if (collapsed) {
        return (
          <div
            ...
            bg-[#0d0f14] border-r border-white/[0.06] py-4 gap-1 shrink-0"
          >
            <button
            ...
            <button
              ...
              text-slate-500 hover:text-slate-200 hover:bg-white/[0.05]
              transition-colors duration-150 bg-transparent border-none
              cursor-pointer" onClick={()=>dispatch(setSelectedConversation(null))}>
              <Plus size={16}/>
            </button>
        ...
        {conversations?.map((conv, i) => {
          ...
          return (
            <div
              key={i}
              onClick={() => {console.log("Clicked:", conv);dispatch(setSelectedConversation(conv))}}
    ...
    <div className={`fixed lg:static inset-y-0 left-0 z-50 w-[270px] h-screen shrink-0 bg-[#0d0f14] border-r border-white/[0.06] transition-transform duration-250 ${
      ...
      <div className='flex flex-col h-full'>
        ...
        <div className='flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]'>
          <div className='hidden lg:flex items-center justify-center w-7 h-7 
                          rounded-lg text-slate-500 hover:text-slate-200
                          hover:bg-white/[0.05] transition-colors duration-150 bg-transparent 
                          border-none cursor-pointer' onClick={() => setCollapsed(!collapsed)}> <PanelLeftIcon />
          </div>

          <button onClick={() => setMobileOpen(false)} className="lg:hidden flex items-center 
          justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200
           hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none 
           cursor-pointer">
            <X />
          </button>
          <span className='text-[16px] font-semibold text-slate-100 tracking-tight flex-1'>Unknown</span>
          <button className='ml-auto flex items-center justify-center w-7 h-7 
                          rounded-lg text-slate-500 hover:text-slate-200
                          hover:bg-white/[0.05] transition-colors duration-150 bg-transparent 
                          border-none cursor-pointer' onClick={()=>dispatch(setSelectedConversation(null))}> <PenSquare size={17}  />
        ...
        <div className="px-4 pt-4 pb-1">
          <button
              ...
              bg-linear-to-br from-indigo-500 to-violet-700 rounded-xl py-[10px]
              border-none cursor-pointer hover:opacity-90 transition-opacity duration-150" onClick={()=>dispatch(setSelectedConversation(null))}><Plus size={15} />
              New Chat
          </button>
        </div>

        {conversations.length==0 ?
        <div className='px-5 pt-4 pb-1.5 text-[10.5px] font-semibold uppercase tracking-widest text-slate-200'>
          ...
          {userData ? (
            <div className="flex items-center gap-2.5 cursor-pointer rounded-xl px-3 py-2.5
              ...
              <div className="flex items-center gap-2.5">
                <button className='text-slate-600 cursor-pointer hover:bg-white/[0.08]
                hover:text-slate-400 transition-all duration-150' onClick={()=>{
                  logout();
